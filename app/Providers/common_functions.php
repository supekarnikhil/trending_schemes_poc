<?php
	...
	function rekeyArrayObject( $strKey, $arrmixData, $boolHasDuplicateKeys = false, $strValColumn = '' ) {
		$arrmixReturnData = [];
		if( $boolHasDuplicateKeys ) {
			foreach( $arrmixData as $object ) {
				$arrmixReturnData[$object->$strKey][] = ( $strValColumn != '' ) ? $object->$strValColumn : $object;
			}
		} else {
			foreach( $arrmixData as $object ) {
				$arrmixReturnData[$object->$strKey] = ( $strValColumn != '' ) ? $object->$strValColumn : $object;
